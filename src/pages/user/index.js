...
import { EditOutlined,PlusCircleOutlined, DeleteOutlined, UserOutlined } from '@ant-design/icons';
...
function Index({dispatch, user, menu}) {
  ...
  const columns=[
    ...
    },
    {
      ...
      key: 'avatar',
      render: (text, record) => (
        <Avatar size={36} icon={<UserOutlined />} src={text} />
      )
    },
    {
      title: '姓名',
    ...
    },
    {
      ...
      render: (text, record) => (
        <div>
          ...
          <Button size='small' type='primary' icon={<DeleteOutlined />} style={{marginLeft:'8px'}} onClick={()=>deleteItem(record.id)} />
        </div>
      )
    },
  ];
      ...
      <Table 
        title={()=>(
          <Link to={pathToRegexp.compile(userEdit)()}>
            <Button size='small' type='primary' icon={<PlusCircleOutlined />}>新增用户</Button>
          </Link>
        )}
        dataSource={data}
        columns={columns}
